...
def setup_song_lyrics_indexes(db):
    ...
    collection.create_index(
        [("title", "text"), ("artist", "text")], name="title_artist_text"
    )
...
def setup_song_gaps_indexes(db):
    ...
    collection.create_index(
        [("song_id", 1), ("difficulty", 1)], unique=True, name="song_difficulty_unique"
    )
...
def setup_user_song_progress_indexes(db):
    ...
    collection.create_index(
        [("user_id", 1), ("is_completed", 1)], name="user_completed_idx"
    )
    logger.info("  ✅ user_id + is_completed")

    # User's recent progress
    collection.create_index(
        [("user_id", 1), ("last_attempt_at", -1)], name="user_recent_attempts_idx"
    )
...
def setup_user_subscription_indexes(db):
    ...
    collection.create_index(
        "subscription_id", unique=True, name="subscription_id_unique"
    )
    logger.info("  ✅ subscription_id (unique)")

    # User's active subscription
    collection.create_index(
        [("user_id", 1), ("is_active", 1)], name="user_active_subscription_idx"
    )
    logger.info("  ✅ user_id + is_active")

    # User's subscription expiry check
    collection.create_index([("user_id", 1), ("end_date", -1)], name="user_expiry_idx")
...
def setup_user_daily_free_songs_indexes(db):
    ...
    collection.create_index(
        [("user_id", 1), ("date", 1)], unique=True, name="user_date_unique"
    )
    logger.info("  ✅ user_id + date (unique)")

    # User's daily usage
    collection.create_index([("user_id", 1), ("songs_count", 1)], name="user_count_idx")
