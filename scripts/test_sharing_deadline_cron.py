...
def main():
    ...
    try:
        ...
        shares_to_remind = list(sharing_service.db.test_shares.find({
            "status": "accepted",
            "deadline": {
        ...
        for share in shares_to_remind:
            try:
                ...
                sharee_name = (
                    sharee.get("name") or
                    sharee.get("display_name") or
                    sharee_email.split("@")[0]
                )
                ...
                print(f"   ❌ Failed to send reminder for share {share.get('share_id')}: {e}")
