...
def main():
    ...
    if correct:
        ...
        print(f"  Has dialogue: {'YES (' + str(len(dialogue)) + ' lines)' if dialogue else 'NO'}")
        if dialogue:
    ...
    db_convs_1 = list(db.conversation_library.find(
        {"topic_number": 1},
    ...
    db_convs_3 = list(db.conversation_library.find(
        {"topic_number": 3},
        {"conversation_id": 1, "conversation_index": 1, "title_en": 1}
    ).sort("conversation_index", 1))
    ...
    if db_convs_3:
        print(f"    ❌ DB indexes: {db_convs_3[0]['conversation_index']} - {db_convs_3[-1]['conversation_index']}")
        print(f"    ❌ Should be: 1 - 20")
        
        # Show first 3 DB conversations
        print("\n    First 3 in DB:")
        for db_conv in db_convs_3[:3]:
            print(f"      [{db_conv['conversation_index']:03d}] {db_conv.get('title_en', 'N/A')}")
