...
def test_upload_regular_image():
    ...
    response = requests.post(
        f"{BASE_URL}/api/library/upload", headers=HEADERS, files=files, data=data
    )
...
def test_convert_to_secret(library_id):
    ...
    encrypted_exif = base64.b64encode(
        b'{"gps": "encrypted", "camera": "encrypted"}'
    ).decode()
...
def test_verify_encryption(library_id):
    ...
    response = requests.get(
        f"{BASE_URL}/api/library/files/{library_id}", headers=HEADERS
    )
    ...
    if response.status_code == 200:
        ...
        if result.get("is_encrypted"):
            print(
                f"Encrypted File Keys: {len(result.get('encrypted_file_keys', {}))} user(s)"
            )
...
def test_non_image_conversion():
    ...
    upload_response = requests.post(
        f"{BASE_URL}/api/library/upload", headers=HEADERS, files=files
    )
