...
# Script deploy hoàn chỉnh cho AI Chatbot RAG với Docker Network, Authentication và System Templates
set -e

echo "🚀 Starting deployment with Docker Network, authentication and system templates..."
...
echo "   • Create production system templates"
echo ""
echo "⚠️  NOTE: This script preserves existing data. Use deploy-fresh-start.sh for clean reset."
echo "📄 This version includes automatic system template creation for production."
echo ""
...
# 12. Create production system templates
echo ""
echo "📄 Creating production system templates..."
if [ -f "create_production_system_templates.py" ]; then
    echo "🔗 Running system template creation with Docker network connectivity..."
    docker run --rm \
      --network "$NETWORK_NAME" \
      --env-file .env \
      -v $(pwd):/app \
      ...
      python:3.10-slim bash -c "
        echo '📦 Installing dependencies...'
        pip install motor pymongo python-dotenv >/dev/null 2>&1 &&
        echo '🚀 Creating production system templates...'
        python create_production_system_templates.py
      "
    echo "✅ System templates creation completed"
else
    echo "⚠️  create_production_system_templates.py not found - skipping template creation"
fi

echo ""
echo "🎉 Setup complete! Your AI Chatbot RAG system is updated and ready."
echo "📋 All existing data has been preserved."
echo "📄 Production system templates have been created."
