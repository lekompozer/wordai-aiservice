...
def migrate_add_point_system_fields():
    ...
    needs_stats = books_collection.count_documents({"stats": {"$exists": False}})
    ...
    if needs_access_config == 0 and needs_stats == 0:
        logger.info(
            "✅ No books need migration. All books already have point system fields."
        )
        return
    ...
    if needs_access_config > 0:
        ...
        result = books_collection.update_many(
            {"access_config": {"$exists": False}},
            {"$set": {"access_config": None}},  # Default: no point-based access
        )

        logger.info(
            f"✅ Successfully added access_config to {result.modified_count} books"
        )
    ...
    remaining_stats = books_collection.count_documents({"stats": {"$exists": False}})

    if remaining_access_config == 0 and remaining_stats == 0:
        logger.info("✅ Migration completed successfully!")
    else:
        logger.warning(
            f"⚠️ Warning: {remaining_access_config} books still need access_config"
        )
        logger.warning(f"⚠️ Warning: {remaining_stats} books still need stats")

    # Show sample data
    sample = books_collection.find_one(
        {}, {"_id": 0, "book_id": 1, "title": 1, "access_config": 1, "stats": 1}
    )
