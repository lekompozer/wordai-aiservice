...
def migrate_guides_to_books():
    ...
    if "online_books" in db.list_collection_names():
        ...
        result = db.online_books.update_many(
            {"guide_id": {"$exists": True}}, {"$rename": {"guide_id": "book_id"}}
        )
        ...
        print(
            f"✅ Added new fields (access_config, community_config, stats): {result.modified_count} documents"
        )
    else:
    ...
    if "book_chapters" in db.list_collection_names():
        ...
        print(
            f"✅ Added new fields (content_source, document_id): {result.modified_count} documents"
        )
    else:
    ...
    if "online_books" in db.list_collection_names():
        ...
        try:
            ...
            print(
                "✅ Created index: online_books.community_config.is_public + published_at"
            )
        except Exception as e:
    ...
    if "online_books" in collections:
        ...
        with_access_config = db.online_books.count_documents(
            {"access_config": {"$exists": True}}
        )
    ...
    if "book_chapters" in collections:
        ...
        with_guide_id = db.book_chapters.count_documents(
    ...
    if "book_permissions" in collections:
        ...
        with_book_id = db.book_permissions.count_documents(
            {"book_id": {"$exists": True}}
        )
        with_guide_id = db.book_permissions.count_documents(
            {"guide_id": {"$exists": True}}
        )
        print(f"   - book_id field: {with_book_id} documents")
        print(f"   - guide_id field (old): {with_guide_id} documents (should be 0)")
