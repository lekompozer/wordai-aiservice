...
**Required Fields:**
- `map_url`: URL to map/diagram image
- `positions`: Array of positions to label (min 1)
  - Each: `{key: "1", x: 100, y: 200, label: "Position 1"}`
- `correct_labels`: Correct labels for positions
...
```json
{
  ...
  "points": 5,
  "map_url": "https://storage.com/cell-diagram.jpg",
  "positions": [
    {"key": "1", "x": 150, "y": 100, "label": "Nucleus"},
    {"key": "2", "x": 250, "y": 150, "label": "Mitochondria"}
  ],
  "correct_labels": {
    "1": "Nucleus",
    "2": "Mitochondria"
...
| Question Type | Required Fields | Optional | Min Items |
|--------------|----------------|----------|-----------|
| MCQ | `question_text`, `options`, `correct_answers` | `explanation`, `media_*` | 2 options |
| Essay | `question_text` | `word_limit_*`, `rubric` | - |
| Matching | `left_items`, `right_options`, `correct_answers` | - | 2 left, 2 right |
| Map Labeling | `map_url`, `positions`, `correct_labels` | - | 1 position |
| Completion | `template`, `blanks`, `correct_answers` | `case_sensitive` | 1 blank |
| Sentence Completion | `sentences` (with `template`, `correct_answers`) | `case_sensitive` | 1 sentence |
| Short Answer | `questions` (with `text`, `correct_answers`) | `case_sensitive` | 1 question |
| Listening | `audio_sections` | - | - |
