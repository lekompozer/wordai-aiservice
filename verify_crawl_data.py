...
def verify_crawl_data():
    ...
    required_fields = [
        "song_id", "title", "artist", "category",
        "english_lyrics", "vietnamese_lyrics",
        "youtube_url", "youtube_id", "source_url"
    ]
    ...
    songs_with_lyrics = list(collection.find(
        {"english_lyrics": {"$ne": ""}},
        {"english_lyrics": 1, "word_count": 1}
    ).limit(1000))  # Sample
    ...
    quality_checks = {
        ...
        "Has English lyrics": collection.count_documents({"english_lyrics": {"$ne": ""}}),
        "Has Vietnamese lyrics": collection.count_documents({"vietnamese_lyrics": {"$ne": ""}}),
        "Has YouTube": collection.count_documents({"youtube_id": {"$ne": ""}}),
    }
    
    total_checks = len(quality_checks)
    passed_checks = sum(1 for count in quality_checks.values() if count / total_songs > 0.95)
