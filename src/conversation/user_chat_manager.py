...
class UserChatManager:
    ...
    def __init__(
        ...
        if not OPENAI_AVAILABLE:
            raise ImportError("OpenAI not available. Please install: pip install openai")
    ...
    async def get_user_sessions(self, user_id: str) -> List[str]:
        ...
        try:
            session_ids = await self.redis_client.smembers(self.user_sessions_key(user_id))
    ...
    async def delete_session(self, session_id: str) -> bool:
        ...
        try:
            ...
            if session:
                ...
                await self.redis_client.srem(self.user_sessions_key(session.user_id), session_id)
    ...
    async def retrieve_context(self, user_id: str, query: str, max_results: int = 5) -> RAGContext:
        """
        ...
        try:
            ...
            retrieval_score = sum(r.score for r in search_results) / len(search_results) if search_results else 0.0
        ...
        try:
            ...
            try:
                ...
                assistant_msg = ChatMessage(
                    ...
                    metadata={
                        "rag_used": use_rag,
                        "rag_documents": rag_context.used_documents if rag_context else [],
                        "rag_score": rag_context.retrieval_score if rag_context else 0.0
                    }
                )
