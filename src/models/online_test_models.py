...
class MCQTypeDistribution(BaseModel):
    ...
    3. matching: Match left items to right options (IELTS style)
    4. completion: Fill in blanks in form/note/table (IELTS style)
    ...
    def validate_distribution(self):
        ...
        if self.distribution_mode == "manual":
            # Check at least one type specified
            types = [
                self.num_single_answer_mcq,
                self.num_multiple_answer_mcq,
                self.num_matching,
                self.num_completion,
