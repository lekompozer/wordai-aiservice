...
class ChatFileContext(BaseModel):
    ...
    filePath: Optional[str] = Field(
        None, description="File path or URL (for PDF with Gemini)"
    )
...
class AIChatRequest(BaseModel):
    ...
    # Conversation ID for saving history (optional - will be generated if not provided)
    conversationId: Optional[str] = Field(
        None, description="Conversation ID for tracking chat history"
    )
