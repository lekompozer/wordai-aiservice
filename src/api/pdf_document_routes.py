...
async def upload_pdf_with_ai(
    ...
    try:
        ...
        if document_type not in ["doc", "slide"]:
            raise HTTPException(
                status_code=400,
                detail="document_type must be 'doc' or 'slide'"
            )
        ...
        if not (1 <= chunk_size <= 10):
            raise HTTPException(
                status_code=400,
                detail="chunk_size must be between 1 and 10"
            )
        ...
        try:
            ...
            logger.info(
                f"✅ Upload complete: {document_id} ({total_time:.2f}s total)"
            )
...
async def preview_document_split(
    ...
    try:
        ...
        if not pdf_r2_path:
            raise HTTPException(
                status_code=400,
                detail="Document does not have PDF file"
            )
        ...
        try:
            ...
            if request.mode == "auto":
                ...
                for idx, chunk_path in enumerate(chunk_files):
                    ...
                    split_parts.append(SplitDocumentPart(
                        document_id=part_doc_id,
                        title=part_title,
                ...
                is_valid, error_msg = pdf_split_service.validate_split_ranges(
                    total_pages,
                    [r.dict() for r in request.split_ranges]
                )
                
                if not is_valid:
                    raise HTTPException(
                        status_code=400,
                        detail=f"Invalid split ranges: {error_msg}"
                    )
                ...
                for idx, range_info in enumerate(request.split_ranges):
                    ...
                    part_pdf_path = os.path.join(
                        tempfile.gettempdir(),
                        f"{document_id}_part{part_num}.pdf"
                    )
                    ...
                    part_document = {
                        ...
                        "user_id": user_id,
                        "title": part_title,
                        "description": range_info.description or f"Pages {range_info.start_page}-{range_info.end_page}",
                        "document_type": document["document_type"],
...
async def convert_document_with_ai(
    ...
    try:
        ...
        logger.info(
            ...
            f"target={request.target_type}, force={request.force_reprocess}"
        )
        ...
        try:
            ...
            logger.info(f"✅ Convert complete ({total_time:.2f}s)")
...
async def get_document_split_info(
    ...
    try:
        ...
        siblings_docs = list(db_manager.db.documents.find({
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Get split info failed: {str(e)}"
        )
...
async def merge_documents(
    ...
    try:
        ...
        # Get all documents
        documents = list(db_manager.db.documents.find({
        ...
        for doc in documents:
            ...
            if doc.get("is_split_part") and doc.get("split_info"):
                pages = f"{doc['split_info']['start_page']}-{doc['split_info']['end_page']}"
            else:
                pages = f"1-{doc.get('total_pages', 0)}"
            
            merged_from_info.append(MergedFromInfo(
