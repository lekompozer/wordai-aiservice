...
async def import_tiktok(
    ...
    if "tiktok.com" not in url:
        raise HTTPException(
            status_code=400, detail="Chỉ hỗ trợ TikTok URL. YouTube sẽ được thêm sau."
        )
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        logger.info(
            f"[music-import] uploading {len(mp3_bytes)//1024}KB to R2: {r2_key}"
        )
        try:
            audio_url = await _upload_to_r2(mp3_bytes, r2_key)
        except Exception as e:
