...
class GenerateTestRequest(BaseModel):
    ...
    user_query: str = Field(..., description="Description of what to test", min_length=10, max_length=500)
    num_questions: int = Field(..., description="Number of questions", ge=1, le=100)
    time_limit_minutes: int = Field(30, description="Time limit in minutes", ge=1, le=300)
...
class SubmitTestRequest(BaseModel):
    ...
    user_answers: list = Field(..., description="List of {question_id, selected_answer_key}")
...
async def generate_test(
    ...
    **Phase 1 Feature**
    ...
    try:
        ...
        logger.info(f"   Questions: {request.num_questions}, Time: {request.time_limit_minutes}min")
    ...
    **Phase 1 Feature**
    ...
    try:
        ...
        if max_retries != "unlimited" and attempts_count >= max_retries:
            raise HTTPException(
                status_code=429,
                detail=f"Maximum attempts ({max_retries}) exceeded"
            )
        ...
        return {
            ...
            "attempts_remaining": max_retries - attempts_count if max_retries != "unlimited" else "unlimited",
        }
...
async def submit_test(
    ...
    **Phase 1 Feature**
    ...
    try:
        ...
        submissions_collection = mongo_service.db["test_submissions"]
        attempt_number = submissions_collection.count_documents({
            "test_id": test_id,
            "user_id": user_info["uid"],
        }) + 1
    ...
    **Phase 1 Feature**
    ...
    try:
        ...
        for test in tests:
            ...
            attempts_count = submissions_collection.count_documents({"test_id": test_id})
