...
Endpoints:
  ...
  GET  /api/v1/podcasts/{podcast_id}/vocabulary      — Vocab + grammar (auth, FREE)
...
Access model:
  - Vocabulary/grammar: FREE for all authenticated users (pre-generated by bulk enrich script)
  - Gap exercises: Premium (Song/Conversation subscription) = unlimited
                   Free users = 3 unique podcasts/day (UTC reset, tracked in user_daily_submits.podcast_ids)
...
# ---------------------------------------------------------------------------
# ENDPOINT 3: Vocabulary & Grammar (auth, FREE)
# ---------------------------------------------------------------------------
...
async def get_podcast_vocabulary(
    ...
    Access: FREE for all authenticated users — no points deducted.
    Content is pre-generated by the bulk enrich script; generated on-demand as fallback.
    """
