...
class StudyHubSubjectManager:
    ...
    def _serialize_subject(
        self, subject: Dict, include_user_context: bool = True
    ) -> Dict:
    ...
    async def get_subject(
        ...
        if include_stats:
            ...
            learner_count = self.enrollments.count_documents(
                {"subject_id": subject["_id"], "status": {"$ne": "dropped"}}
            )
    ...
    async def update_subject(
        ...
        # Update subject
        self.subjects.update_one({"_id": subject["_id"]}, {"$set": update_data})
    ...
    async def list_subjects(
        ...
        if not owner_id or (owner_id != self.user_id):
            if not self.user_id:
                ...
                enrolled_subject_ids = [
                    e["subject_id"]
                    for e in self.enrollments.find(
                        {"user_id": self.user_id, "status": {"$ne": "dropped"}}
                    )
                ]
                query["$or"] = [
                    {
                        "visibility": SubjectVisibility.PUBLIC.value,
        ...
        skip = (page - 1) * limit
        sort_field = (
            sort if sort in ["created_at", "updated_at", "title"] else "created_at"
        )
    ...
    async def get_owner_subjects(
        ...
        subjects = list(
            self.subjects.find(query).sort("created_at", -1).skip(skip).limit(limit)
        )
