...
class TestCoverImageService:
    ...
    def __init__(self):
        ...
        if not PIL_AVAILABLE:
            raise ImportError("Pillow required for image processing. Install: pip install Pillow")
        ...
        self.cdn_base_url = getattr(config, 'R2_PUBLIC_URL', f"https://pub-{config.R2_ACCOUNT_ID}.r2.dev")
        
        logger.info("✅ TestCoverImageService initialized")
    
    async def validate_image(self, image_bytes: bytes, filename: str) -> Tuple[bool, Optional[str], Optional[Image.Image]]:
        """
        ...
        try:
            ...
            if image.format not in self.ALLOWED_FORMATS:
                return False, f"Invalid format: {image.format}. Allowed: {', '.join(self.ALLOWED_FORMATS)}", None
            
            # Check dimensions
            width, height = image.size
            if width < self.MIN_WIDTH or height < self.MIN_HEIGHT:
                return False, f"Image too small: {width}x{height}. Minimum: {self.MIN_WIDTH}x{self.MIN_HEIGHT}", None
            
            logger.info(f"✅ Image validation passed: {filename} ({width}x{height}, {image.format})")
    ...
    def optimize_image(self, image: Image.Image, max_dimension: int = 1920) -> bytes:
        ...
        try:
            ...
            if width > max_dimension or height > max_dimension:
                ...
                logger.info(f"🔽 Resized image: {width}x{height} → {new_size[0]}x{new_size[1]}")
            ...
            logger.info(f"✅ Optimized image: {len(optimized_bytes.getvalue()) / 1024:.0f}KB")
    ...
    async def upload_cover_image(
        ...
        try:
            # Validate image
            is_valid, error_msg, image = await self.validate_image(image_bytes, filename)
            if not is_valid:
            ...
            cover_filename = f"test-covers/{test_id}/{version}/cover_{timestamp}_{unique_id}.{ext}"
            thumbnail_filename = f"test-covers/{test_id}/{version}/thumb_{timestamp}_{unique_id}.{ext}"
            ...
            if not thumbnail_success:
                ...
                logger.warning(f"⚠️  Thumbnail upload failed, but cover uploaded successfully")
            
            # Generate CDN URLs
            cover_url = f"{self.cdn_base_url}/{cover_filename}"
            thumbnail_url = f"{self.cdn_base_url}/{thumbnail_filename}" if thumbnail_success else cover_url
