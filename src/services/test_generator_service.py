...
class TestGeneratorService:
    ...
    def __init__(self):
        ...
        self.max_retries = 3
        logger.info("🤖 Test Generator Service initialized (Gemini 2.5 Pro with JSON Mode)")
    ...
    async def generate_test_from_content(
        ...
        try:
            ...
            for attempt in range(self.max_retries):
                try:
                    logger.info(f"   Attempt {attempt + 1}/{self.max_retries}: Calling Gemini...")
                    ...
                    if hasattr(response, "text") and response.text:
                        ...
                        logger.info(f"   ✅ Gemini response: {len(response_text)} characters")
                    else:
                    ...
                    if not isinstance(questions_json, dict) or "questions" not in questions_json:
                        raise ValueError("Invalid JSON structure: missing 'questions' field")
                    
                    questions_list = questions_json["questions"]
                    if not isinstance(questions_list, list) or len(questions_list) == 0:
                        raise ValueError("Invalid JSON: 'questions' must be a non-empty array")
                    
                    # Validate each question
                    for idx, q in enumerate(questions_list):
                        if not all(k in q for k in ["question_text", "options", "correct_answer_key", "explanation"]):
                    ...
                    is_retryable = any(keyword in error_str for keyword in [
                        ...
                        'timeout', 'overload', '529', 'resource exhausted'
                    ])
