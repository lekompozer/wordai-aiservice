...
class StudyHubModuleManager:
    ...
    async def create_module(
        ...
        last_module = self.modules.find_one(
            {"subject_id": subject_oid}, sort=[("order_index", -1)]
        )
        ...
        self.subjects.update_one(
            {"_id": subject_oid},
            {"$inc": {"metadata.total_modules": 1}, "$set": {"updated_at": now}},
        )
    ...
    async def get_modules(self, subject_id: str) -> List[ModuleResponse]:
        ...
        modules = list(
            self.modules.find({"subject_id": subject_oid}).sort("order_index", 1)
        )
    ...
    async def update_module(
        ...
        # Update module
        self.modules.update_one({"_id": module_oid}, {"$set": update_data})
    ...
    async def delete_module(self, module_id: str) -> bool:
        ...
        remaining_modules = list(
            self.modules.find({"subject_id": module["subject_id"]}).sort(
                "order_index", 1
            )
        )

        for idx, mod in enumerate(remaining_modules):
            self.modules.update_one({"_id": mod["_id"]}, {"$set": {"order_index": idx}})
        ...
        logger.info(
            f"Deleted module {module_id} and re-indexed {len(remaining_modules)} modules"
        )
    ...
    async def reorder_module(
        ...
        logger.info(
            f"Reordered module {module_id} from {old_index} to {new_order_index}"
        )
    ...
    async def add_content(
        ...
        last_content = self.contents.find_one(
            {"module_id": module_oid}, sort=[("order_index", -1)]
        )
        ...
        # Update module
        self.modules.update_one({"_id": module_oid}, {"$set": {"updated_at": now}})
    ...
    async def get_contents(self, module_id: str) -> List[ModuleContentResponse]:
        ...
        contents = list(
            self.contents.find({"module_id": module_oid}).sort("order_index", 1)
        )
    ...
    async def delete_content(self, module_id: str, content_id: str) -> bool:
        ...
        content = self.contents.find_one({"_id": content_oid, "module_id": module_oid})
        ...
        remaining_contents = list(
            self.contents.find({"module_id": module_oid}).sort("order_index", 1)
        )

        for idx, cnt in enumerate(remaining_contents):
            self.contents.update_one(
                {"_id": cnt["_id"]}, {"$set": {"order_index": idx}}
            )

        # Update module
        self.modules.update_one(
            {"_id": module_oid}, {"$set": {"updated_at": datetime.now(timezone.utc)}}
        )

        logger.info(
            f"Deleted content {content_id} and re-indexed {len(remaining_contents)} contents"
        )
