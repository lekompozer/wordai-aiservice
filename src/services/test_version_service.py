...
class TestVersionService:
    ...
    def get_current_version(self, test_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if version_doc:
                # Convert ObjectId to string
                version_doc["_id"] = str(version_doc["_id"])
                logger.info(f"✅ Retrieved current version: {test_id} {version_doc.get('version')}")
            else:
    ...
    def list_versions(
        ...
        try:
            ...
            versions = list(self.db.test_versions.find(
                {"test_id": test_id},
                projection
            ).sort("version_number", -1))
    ...
    def get_version_for_purchase(self, test_id: str, purchased_at: datetime) -> Optional[str]:
        """
        ...
        try:
            ...
            if version_doc:
                ...
                logger.warning(f"⚠️  No version found at purchase time, defaulting to v1")
                return "v1"
        ...
        try:
            ...
            comparison = {
                ...
                "changes": {
                    ...
                    "questions_count_changed": len(v1_doc.get("questions", [])) != len(v2_doc.get("questions", [])),
                    "time_limit_changed": v1_doc.get("time_limit_minutes") != v2_doc.get("time_limit_minutes"),
                    "max_retries_changed": v1_doc.get("max_retries") != v2_doc.get("max_retries"),
                    "price_changed": v1_doc.get("marketplace_config_snapshot", {}).get("price_points") != v2_doc.get("marketplace_config_snapshot", {}).get("price_points")
                },
                "v1": {
                    "title": v1_doc.get("title"),
                    "questions_count": len(v1_doc.get("questions", [])),
                    "time_limit_minutes": v1_doc.get("time_limit_minutes"),
                    "price_points": v1_doc.get("marketplace_config_snapshot", {}).get("price_points")
                },
                "v2": {
                    "title": v2_doc.get("title"),
                    "questions_count": len(v2_doc.get("questions", [])),
                    "time_limit_minutes": v2_doc.get("time_limit_minutes"),
                    "price_points": v2_doc.get("marketplace_config_snapshot", {}).get("price_points")
                }
            }
