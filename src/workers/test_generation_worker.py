...
class TestGenerationWorker:
    ...
    async def process_listening_test(self, task_data: dict):
        ...
        try:
            ...
            # 🔧 FILTER VALIDATION: Remove invalid questions (AI hallucinations)
            ...
            for idx, q in enumerate(raw_questions):
                ...
                # Skip invalid question types (AI hallucinations)
                if q_type not in valid_question_types:
                    logger.warning(
                        f"⚠️ Skipping question {idx + 1}: Invalid question_type='{q_type}'"
                    )
                    logger.warning(f"   Question data: {q}")
                    continue

                # Skip questions without question_text
                if not q.get("question_text"):
                    logger.warning(
                        f"⚠️ Skipping question {idx + 1}: Missing question_text"
                    )
                    continue
            ...
            logger.info(
                f"✅ Validated {len(valid_questions)}/{len(raw_questions)} questions"
            )
