...
def main():
    ...
    for topic_num in range(1, 31):
        ...
        report.append(f"\n{'='*80}")
        report.append(
            f"TOPIC {topic_num}: {TOPICS[topic_num-1]['en']} ({TOPICS[topic_num-1]['slug']})"
        )
        report.append(f"{'='*80}")
        ...
        if db_convs:
            ...
            for i, db_conv in enumerate(db_convs):
                ...
                if db_slug != expected_slug:
                    issues.append(
                        f"❌ Wrong slug: '{db_slug}' (expected '{expected_slug}')"
                    )
        ...
        if missing_in_db:
            ...
            for idx in missing_in_db:
                ...
                report.append(
                    f"  [{idx:03d}] {file_conv['title_en']} | {file_conv['title_vi']}"
                )
