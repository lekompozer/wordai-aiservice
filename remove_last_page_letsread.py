...
def remove_last_page_for_book(db, book_id: str, title: str, dry_run: bool = False) -> bool:
...
def main():
    parser = argparse.ArgumentParser(description="Remove last (promo) page from LetsRead books")
    parser.add_argument("--dry-run", action="store_true", help="Preview only, no DB writes")
    parser.add_argument("--book-id", help="Process only this specific book ID", default=None)
    ...
    if args.book_id:
        ...
        book = db.online_books.find_one(
            {"_id": __import__("bson").ObjectId(args.book_id)},
            {"title": 1}
        )
        if not book:
            log.error(f"Book {args.book_id} not found")
            sys.exit(1)
        books = [{"_id": book["_id"], "title": book.get("title", "?"), "id_str": args.book_id}]
    else:
        # Find all LetsRead books by source
        raw = list(db.online_books.find(
            {"metadata.source": "letsreadasia.org"},
            {"title": 1, "metadata.letsread_book_id": 1}
        ))
        books = [{"_id": b["_id"], "title": b.get("title", "?"), "id_str": str(b["_id"])} for b in raw]

    log.info(f"{'[DRY-RUN] ' if args.dry_run else ''}Processing {len(books)} letsread books...")
    log.info("")
    ...
    for book in books:
        ...
        # Count pages
        total_pages = db.book_page_texts.count_documents({"book_id": book_id, "language": "en"})
