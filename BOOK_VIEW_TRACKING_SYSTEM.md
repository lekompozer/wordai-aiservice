...
- If yes → Skip (no increment)
- If no → Record session + increment `community_config.total_views`
...
```
1. Validate inputs
   ├─ If no user_id AND no browser_id → Return False
   └─ Continue

2. Check if community book
   ├─ Query: {book_id, "community_config.is_public": true}
   ├─ If not found → Return False (not a community book)
   └─ Continue

3. Create viewer_id
   ├─ If user_id → viewer_id = user_id
   └─ If browser_id → viewer_id = f"browser_{browser_id}"

4. Get today's date range (UTC)
   ├─ today_start = 2025-11-20 00:00:00
   └─ today_end = 2025-11-21 00:00:00

5. Check existing view today
   ├─ Query: {book_id, viewer_id, viewed_at: {$gte: today_start, $lt: today_end}}
   ├─ If found → Return False (already viewed today)
   └─ Continue

6. Record new view
   ├─ Insert into book_view_sessions
   └─ Increment: online_books.community_config.total_views + 1
...
✅ Connected to database: wordai

📋 Creating indexes for book_view_sessions collection...
✅ Created index: book_viewer_date_unique (book_id, viewer_id, viewed_at)
✅ Created TTL index: view_session_ttl (expires_at)
✅ Created index: book_views_by_date (book_id, viewed_at)
✅ Created index: user_views_by_date (user_id, viewed_at)
...
**Estimated Load:**
- 1000 unique users/day × 5 books/user = 5000 documents/day
...
// If found → View not counted (deduplication)
```
