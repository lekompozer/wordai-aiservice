...
def migrate_fix_missing_authors():
    ...
    print(f"📡 Connecting to MongoDB: {MONGODB_URI.replace(MONGODB_URI.split('@')[0].split('//')[1], '***')}")
    ...
    try:
        ...
        published_books = list(db.online_books.find({
        ...
        for book in published_books:
            ...
            if not authors_list:
                print(f"⚠️  Book {book_id} ({title}) has no authors field!")
            ...
            for author_id in authors_list:
                ...
                if existing_author:
                    ...
                    # Create new author
                    author_name = author_id.replace("@", "").replace("_", " ").replace("-", " ").title()
                    ...
                    print(f"✨ Created new author: {author_id} ({author_name}) for book {book_id}")
                    authors_created += 1
        
        print("\n" + "="*60)
        print("📊 MIGRATION SUMMARY:")
        print(f"   Published books: {len(published_books)}")
        print(f"   Books without authors field: {books_without_authors}")
