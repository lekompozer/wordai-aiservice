...
async def fix_merged_audio(presentation_id: str, language: str):
    ...
    for chunk in chunks:
        ...
        if (
            idx not in latest_chunks
            or chunk["created_at"] > latest_chunks[idx]["created_at"]
        ):
    ...
    for chunk in audio_docs:
        print(
            f"      Chunk {chunk['chunk_index']}: {len(chunk.get('slide_timestamps', []))} slides"
        )
    ...
    voice_config = audio_docs[0].get(
        "voice_config", {"voice_name": "vi-VN-Neural2-D", "language_code": "vi-VN"}
    )
    ...
    try:
        ...
        print(
            f"   First slide: {timestamps[0]['start_time']:.1f}s → {timestamps[0]['end_time']:.1f}s"
        )
        print(
            f"   Last slide: {timestamps[-1]['start_time']:.1f}s → {timestamps[-1]['end_time']:.1f}s"
        )
        print(
            f"   Audio duration: {merged_doc['audio_metadata']['duration_seconds']:.1f}s"
        )
        ...
        for i in range(len(timestamps) - 1):
            ...
            if next_start < current_end:
                print(
                    f"   ⚠️ Overlap: Slide {i} ends at {current_end:.1f}s, Slide {i+1} starts at {next_start:.1f}s"
                )
                has_issues = True
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            "Usage: python fix_merged_audio_timestamps.py <presentation_id> <language>"
        )
