...
def create_conversation_indexes():
    ...
    collection.create_index(
        [("conversation_id", ASCENDING)], unique=True, name="idx_conversation_id"
    )
    ...
    # Search by topic
    collection.create_index([("topic_slug", ASCENDING)], name="idx_topic_slug")
    print("  ✓ idx_topic_slug")

    # Search by level + topic
    collection.create_index(
        [("level", ASCENDING), ("topic_slug", ASCENDING)], name="idx_level_topic"
    )
    print("  ✓ idx_level_topic")

    # Search by difficulty
    collection.create_index([("difficulty_score", ASCENDING)], name="idx_difficulty")
    ...
    # Primary key
    collection.create_index([("vocab_id", ASCENDING)], unique=True, name="idx_vocab_id")
    ...
    # Primary key
    collection.create_index([("gap_id", ASCENDING)], unique=True, name="idx_gap_id")
    ...
    # Search by difficulty
    collection.create_index([("difficulty", ASCENDING)], name="idx_difficulty")
    print("  ✓ idx_difficulty")
    ...
    # Primary key
    collection.create_index(
        [("progress_id", ASCENDING)], unique=True, name="idx_progress_id"
    )
    ...
    collection.create_index([("last_attempt_at", DESCENDING)], name="idx_last_attempt")
    print("  ✓ idx_last_attempt")

    # Search by completion status
    collection.create_index([("is_completed", ASCENDING)], name="idx_is_completed")
    print("  ✓ idx_is_completed")

    # User's completed conversations
    collection.create_index(
        [("user_id", ASCENDING), ("is_completed", ASCENDING)], name="idx_user_completed"
    )
